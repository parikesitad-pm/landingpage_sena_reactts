...
async function optimizeImages() {
  console.log("Starting image optimization with Sharp (stripping EXIF/GPS metadata)...");
  ...
  for (const item of imageMappings) {
    ...
    console.log(`Processing ${item.source} (${originalWidth}px) -> ${item.destBase}`);
    ...
    await sharp(sourcePath)
      .resize({ width: Math.min(originalWidth, 1280), withoutEnlargement: true })
      .webp({ quality: 82 })
